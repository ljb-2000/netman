...
*/
$tsql = "SELECT COUNT(DISTINCT Tag) as totalCount FROM Events";
if($c > 0){
	$where .= " AND ";
}
else{
...
*/
$tagssql = "SELECT DISTINCT Tag, SUM(Count) as Total,MIN(FirstSeen) AS FirstSeen,MAX(LastSeen) AS LastSeen  FROM Events $where GROUP BY Tag $sort $limit";
foreach($db->query($tagssql) as $row){
	$tags[] = $row;
}
